...
import { db } from "@trinity/db";
import { authConfig } from "@trinity/auth";

import { makeAuth, Session } from "@koons/auth";
...
export const createTRPCContext = async (opts: {
    ...
    const session = opts.session ?? (await makeAuth(authConfig)(opts.headers));
    const source = opts.headers.get("x-trpc-source") ?? "unknown";
    const agent = opts.headers.get("user-agent") ?? "unknown";
